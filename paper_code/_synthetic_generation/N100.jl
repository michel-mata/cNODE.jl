...
for (d,D) in enumerate(parameters)
    ...
    for (e,p) in inx
        ...
        CSV.write("./Data/Synthetic/$N/$D/A$(p-1)_$e.csv",convert(DataFrame,hcat(A₀,r)); header=false)
        CSV.write("./Data/Synthetic/$N/$D/Z$(p-1)_$e.csv",convert(DataFrame,Z); header=false)
        CSV.write("./Data/Synthetic/$N/$D/P$(p-1)_$e.csv",convert(DataFrame,X); header=false)
    end
end

...
@everywhere begin
    ...
    function split_data(Z,P,p)
        (ztrn,ptrn),(ZT,PT) = splitobs(shuffleobs((Z,P)),at = 0.8)
        (ztrn,ptrn),_ = splitobs(shuffleobs((ztrn,ptrn)),at = p - 1e-3)
    ...
    max_epochs = 1000
    early_stoping = 100
    percentage = [.01,0.1,0.25,0.5,0.75,1]
    parameters = 0:2
    nets = 0:9
end
...
begin
    ...
    root = "./test_results/Hyperparameters/"
    Validation =  []
    ...
    @sync @distributed for it in inx
        ...
        @time for p in 1:length(percentage)
            ...
            if percentage[p]==percentage[end]
                writedlm("./test_results/$DATA/loss_epochs/train_$i$j.csv",loss_train, ',')
                writedlm("./test_results/$DATA/loss_epochs/val_$i$j.csv",loss_val, ',')
                writedlm("./test_results/$DATA/loss_epochs/test_$i$j.csv",loss_test, ',')
            end

            "--- $(percentage[p]) ---" |> println
        end
        # Export
        writedlm("./test_results/$DATA/loss_percentage/train_$i$j.csv",LossTrain, ',')
        writedlm("./test_results/$DATA/loss_percentage/val_$i$j.csv",LossVal, ',')
        writedlm("./test_results/$DATA/loss_percentage/test_$i$j.csv",LossTest, ',')
    end
end
#

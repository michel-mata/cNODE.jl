...
for (j,DATA) in enumerate(Datasets)
    ...
    for (i,fold) in enumerate(kfolds((Z,P); k = M))
        ...
        @time for e in 1:epochs
            ...
            for mb in partition(randperm(size(ztrn,2)), mb_size)
                l = LossR(resnet,ztrn[:,mb],ptrn[:,mb])
                Flux.back!(l)
    ...
    writedlm("./Results/Experimental/Reptile/ResNet/$DATA/real_sample.csv",Ptst, ',')
    writedlm("./Results/Experimental/Reptile/ResNet/$DATA/pred_sample.csv",Qtst, ',')
    writedlm("./Results/Experimental/Reptile/ResNet/$DATA/test_loss.csv",test_loss, ',')
    writedlm("./Results/Experimental/Reptile/ResNet/$DATA/train_loss.csv",train_loss, ',')
end
